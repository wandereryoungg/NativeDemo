        ...
        do {
            ...
            for (size_t i = 0; i < static_cast<size_t>(numEvents); i++) {
                ...
                if (event.header.type ==
                    ...
                    if (oldTimeStamp) {
                        float t = float(event.header.timestamp - oldTimeStamp) /
                                  s2ns(1);
                        printf("%f ms (%f Hz)\n", t * 1000, 1.0 / t);
                    } else {
                        printf("\n");
...
void DisplayEventTracker::processDisplayEvents() {
    // This will poll mDisplayEventReceiver and if there are new events it'll
    // call displayEventCallback synchronously.
    mLooper->pollOnce(-1);
}
