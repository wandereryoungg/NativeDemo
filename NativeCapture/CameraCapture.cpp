...
CameraCapture::CameraCapture() { ALOGI("%s %p", __func__, this); }
...
int CameraCapture::startCapture(int32_t width, int32_t height, int32_t format,
    ...
    ALOGI("%s   mWidth: %d   mHeight: %d   mFormat: %d   mUsage: %" PRIu64
          "   mMaxImages: %d",
          __func__, mWidth, mHeight, mFormat, mUsage, mMaxImages);
...
int CameraCapture::initImageReader() {
    if (mImgReader != nullptr || mImgReaderAnw != nullptr) {
        ALOGE("Cannot re-init image reader, mImgReader=%p, mImgReaderAnw=%p",
    ...
    if (ret != AMEDIA_OK || mImgReaderAnw == nullptr) {
        ALOGE(
            "Failed to get ANativeWindow from AImageReader, ret=%d, "
            "mImgReaderAnw=%p.",
            ret, mImgReaderAnw);
        return -1;
    }
...
void CameraCapture::handleImageAvailable(AImageReader* reader) {
    ...
    if (ret != AMEDIA_OK || outBuffer == nullptr) {
        ALOGE("Failed to get hardware buffer, ret=%d, outBuffer=%p.", ret,
              outBuffer);
        return;
    }
    ...
    if (mFormat == AIMAGE_FORMAT_RGBA_8888 ||
        ...
        mFormat == AIMAGE_FORMAT_YUV_420_888 || mFormat == AIMAGE_FORMAT_Y8) {
        // Check output buffer dimension for certain formats. Don't do this for
        // blob based formats.
        if (bufferWidth != mWidth || bufferHeight != mHeight) {
            ALOGE(
                "Mismatched output buffer dimension: expected=%dx%d, "
                "actual=%dx%d",
                mWidth, mHeight, bufferWidth, bufferHeight);
            return;
        }
    }
    ...
    if (imageWidth != mWidth || imageHeight != mHeight) {
        ALOGE("Mismatched output image dimension: expected=%dx%d, actual=%dx%d",
              mWidth, mHeight, imageWidth, imageHeight);
        return;
    }

    if ((outDesc.usage & mUsage) != mUsage) {
        ALOGE("Mismatched output buffer usage: actual (%" PRIu64
              "), expected (%" PRIu64 ")",
    ...
    if (mUsage & AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN) {
        ...
        if (ret != AMEDIA_OK || data == nullptr || dataLength < 0) {
            ALOGE("Failed to access CPU data, ret=%d, data=%p, dataLength=%d",
                  ret, data, dataLength);
            return;
        }
    } else {
        if (ret != AMEDIA_IMGREADER_CANNOT_LOCK_IMAGE || data != nullptr ||
            dataLength != 0) {
            ALOGE(
                "Shouldn't be able to access CPU data, ret=%d, data=%p, "
                "dataLength=%d",
                ret, data, dataLength);
            return;
        }
    ...
    mAcquiredImageCount++;
    ALOGI("%s   mAcquiredImageCount: %zu   dataLength: %d", __func__,
          mAcquiredImageCount, dataLength);
    if (mAcquiredImageCount == 100) {
        FILE* file;
        char fileName[128];
        sprintf(fileName, "/data/camera/camera_dump_%zu.yuv",
                mAcquiredImageCount);
...
int CameraCapture::initCamera() {
    ...
    ACameraMetadata_const_entry availableFps;
    ACameraMetadata_getConstEntry(
        mCameraMetadata, ACAMERA_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES,
        &availableFps);
    for (int i = 0; i < availableFps.count; i += 2) {
        ALOGI("%s fps range %d-%d", __func__, availableFps.data.i32[i],
              availableFps.data.i32[i + 1]);
    }
    ...
    ret = ACaptureRequest_setEntry_i32(mCaptureRequest,
                                       ACAMERA_CONTROL_AE_TARGET_FPS_RANGE,
                                       fpsRanges.size(), fpsRanges.data());
    if (ret != AMEDIA_OK) {
    ...
    ACameraCaptureSession_captureCallbacks mCaptureCallbacks = {
        ...
        [](void*, ACameraCaptureSession*, ACaptureRequest*,
           const ACameraMetadata*) { ALOGI("%s", __func__); },
    ...
    int reqSeq;
    ACameraCaptureSession_setRepeatingRequest(mSession, &mCaptureCallbacks, 1,
                                              &mCaptureRequest, &reqSeq);
