...
void AudioUtil::createEngine() {
    ...
    // objects
    result = (*engineObject)
                 ->GetInterface(engineObject, SL_IID_ENGINE, &engineEngine);
    assert(SL_RESULT_SUCCESS == result);
    (void)result;
...
void AudioUtil::bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq,
                                 void* context) {
...
void AudioUtil::createBufferQueueAudioPlayer() {
    ...
    SLDataFormat_PCM format_pcm = {
        SL_DATAFORMAT_PCM,           1,
        SL_SAMPLINGRATE_32,          SL_PCMSAMPLEFORMAT_FIXED_16,
    ...
    if (bqPlayerSampleRate) {
        format_pcm.samplesPerSec =
            bqPlayerSampleRate;  // sample rate in mili second
    }
    ...
    /*
     * create audio player:
     *     fast audio does not support when SL_IID_EFFECTSEND is required, skip
     * it for fast audio case
     */
    ...
    result = (*engineEngine)
                 ->CreateAudioPlayer(engineEngine, &bqPlayerObject, &audioSrc,
                                     &audioSnk, bqPlayerSampleRate ? 2 : 3, ids,
                                     req);
    assert(SL_RESULT_SUCCESS == result);
    (void)result;
    // realize the player
...
// like to avoid excessive re-sampling while playing back from Hello & Android
// clip
void AudioUtil::createAudioRecorder() {
    ...
    result = (*recorderBufferQueue)
                 ->RegisterCallback(recorderBufferQueue, bqRecorderCallback,
                                    (void*)recorderRecord);
    assert(SL_RESULT_SUCCESS == result);
    (void)result;
    ALOGI("%s:%d", __FUNCTION__, __LINE__);
...
void AudioUtil::bqRecorderCallback(SLAndroidSimpleBufferQueueItf bq,
                                   void* context) {
    std::unique_lock<std::mutex> lock(audioLock);
    ...
    if (SL_RESULT_SUCCESS == result) {
        ...
        if (file) {
            ALOGI("%s:%d:   write fileName: %s", __FUNCTION__, __LINE__,
                  fileName);
            fwrite(recorderBuffer, 1, recorderSize, file);
            fclose(file);
        } else {
            ALOGI("%s:%d:   fopen fileName: %s error: %s", __FUNCTION__,
