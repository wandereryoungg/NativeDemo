...
static int ALIGN(int x, int y) { return (x + y - 1) & ~(y - 1); }
...
void render(const void *data, size_t size,
    ...
    // v1.0
    (nativeWindow.get())
        ->perform(nativeWindow.get(), NATIVE_WINDOW_SET_BUFFERS_GEOMETRY,
                  bufWidth, bufHeight, halFormat);
#else
    // v2.0
    native_window_set_buffers_dimensions(nativeWindow.get(), bufWidth,
                                         bufHeight);
...
int main(int argc, char *argv[]) {
    ...
    int lcd_height =
        mainDpyInfo.supportedDisplayModes[0].resolution.getHeight();

    sp<SurfaceControl> surfaceControl = client->createSurface(
        String8("TSurface"), lcd_width, lcd_height, PIXEL_FORMAT_RGBA_8888, 0);
    SurfaceComposerClient::Transaction{}
        .setLayer(surfaceControl, 100000)
        .show(surfaceControl)
